...
class CrowdingDistanceAlgorithm(object):
    ...
    def get_min_distance(individual_list, individual):
        ...
        for ind in individual_list:
            if ind.individual_id != individual.individual_id:
                distance = math.sqrt(
                    math.pow((ind.cost - individual.cost), 2) + math.pow((ind.makespan - individual.makespan), 2))
    ...
    def individual_select_by_crowding_distance(self, individual_list, num):
        ...
        cost_distance_dict, first_individual_cost, last_individual_cost = \
            self.individual_sort_by_attr(individual_list, "cost")

        crowding_distance = list()

        makespan_dis_id_list = makespan_distance_dict.keys()
        cost_dis_id_list = cost_distance_dict.keys()

        for indi_id in makespan_dis_id_list:
            if indi_id in cost_dis_id_list:
                distance = makespan_distance_dict[indi_id] + cost_distance_dict[indi_id]
        ...
        for individual in [first_individual_makespan, last_individual_makespan, first_individual_cost,
                           last_individual_cost]:
            exist = False
            for i in prior_list:
