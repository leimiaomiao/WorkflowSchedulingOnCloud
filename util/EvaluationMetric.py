...
class EvaluationMetric(object):
    ...
    def fs_metric(pareto):
        ...
        pareto_sorted_by_cost = sorted(pareto, key=lambda individual: individual.cost)
        min_cost_dif = pareto_sorted_by_cost[1].cost - pareto_sorted_by_cost[0].cost

        index = 0
        while index < len(pareto_sorted_by_cost) - 1:
            dif = pareto_sorted_by_cost[index + 1].cost - pareto_sorted_by_cost[index].cost
            if dif < min_cost_dif:
                min_cost_dif = dif
            index += 1

        return math.sqrt(math.pow(min_makespan_dif, 2) + math.pow(min_cost_dif, 2))
